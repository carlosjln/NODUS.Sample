    ...
    function Piechart(paper, cx, cy, r, values, opts) {
        ...
        if (len == 1) {
            ...
            for (i = 0; i < len; i++) {
				...
				// console.log( values[i], total, values[i] / total );
...
//                opts.init && p.animate({ path: path.join(",") }, (+opts.init - 1) || 1000, ">");
 ...
 var p, path = sector(cx, cy, r, angle, angle -= 360 * values[i] / total);
                ...
                if (values[i].value < total) {
                    ...
                    var p2 = paper.circle(cx, cy, r).attr(chartinst.shim);
...
//                p = paper.path(sectors[i].attr("path")).attr(chartinst.shim);
        ...
        if (donut) {
			...
			var doc_elem_prop = paper.circle(cx, cy, donutRadius).attr({ fill: donutFill, stroke: opts.stroke || donutFill});
			series.push(doc_elem_prop);
			chart.donut = doc_elem_prop;
        }
